...
@bind d Slider(0.0005:0.0005:1, default=0.5)
...
begin
	plot(sin,-π:d:π, title = "Funciones trigonométricas", xlabel = "x", ylabel = "y", color = "red", label = "sin(x)", marker = false)
	plot!(cos,-π:d:π, color = "blue", label = "cos(x)", marker = false)
end
...
a = 6																		            #Definimos la variable "a" como la altura.
r = 4																		            #Definimos la variable "r" como la rapidez.
θ = 45																		            #Definimos la variable "θ" como el angulo.
t = 5																		            #Definimos la variable "t" como el tiempo.
	
	Vrx = r * cos(θ)                                                                    #Definimos el vector velocidad en el eje x.
	Vry = r * sin(θ) 																	#Definimos el vector velocidad en el eje y.	

p(x) = ((Vrx) * x, a + (Vry) * x - ((9.81 * x^2) / 2))                                  #Definimos el vector posición.
py(x) = (a + (Vry) * x - ((9.81 * x^2) / 2))                                            #Definimos la componente y del vector posición.
px(x) = (Vrx * x)                                                                       #Definimos la componente x del vector posición.

	tiro = @animate for t in 0:0.1:5                                                    #Por cada t en el intervalo de 0 a 5 con "pasos" de 0.1.
		plot(p.(range(-(3),0,step = 0.1).+t),                                           
		legend = false, title = "Tiro parabólico", lw = 3, ls=:dot)                     #Creamos una grafica punteada, con la leyenda de "Tiro parabólico".

scatter!([px(t)],[py(t)])                                                               #Graficamos la particula que representa el tiro parabolico.

	x_max = (Vrx * ((2 * Vry/9.81 + ((2 * Vry/9.81)^2 + 8 * a/9.81)^(1/2))/2))          #Calculamos el alcance máximo de la partida horizontalmente.
	y_max = (a + (Vry) * (Vry/9.81) - 9.81 * (Vry/9.81)^2/2)                            #Calculamos el alcance máximo de la partida verticalmente.
		
		xlabel!("Distancia")                                                            #Le damos el nombre de "distancia" al eje x.
		ylabel!("Altura")                                                               #Le damos el nombre de "altura" al eje y.
		xlims!(0,x_max + 0.5)                                                           #Colocamos los límites del eje x.
		ylims!(0,y_max + 0.5)                                                           #Colocamos los límites del eje y.
		
end

	gif(tiro, "Tiro_Parabólico.gif", fps = 30)                                          #Creamos la variable tiro, como un gif llamado "Tiro Parabólico".
...
function parabola(a,r,θ,t)																#Definimos la función que tome parametros a,r,θ,t
...
px(x) = (Vrx * x)                                                                       #Definimos la componente x del vector posición.
	...
	tiro = @animate for t in 0:0.1:5                                                    #Por cada t en el intervalo de 0 a 5 con "pasos" de 0.1.
		plot(p.(range(-(3),0,step = 0.1).+t),                                           
		legend = false, title = "Función de tiro parabólico", lw = 3, ls=:dot)                     #Creamos una grafica punteada, con la leyenda de "Tiro parabólico".
...
scatter!([px(t)],[py(t)])                                                               #Graficamos la particula que representa el tiro parabolico.
	...
	gif(tiro, "Tiro_Parabólico_2.gif", fps = 30)                                          #Creamos la variable tiro, como un gif llamado "Tiro Parabólico".
...
default(legend = false, title = "h(x,y) = cos(x) + sin(y)")       #Generamos una gráfica sin leyenda pero con el titulo de la función pedida.
	
	x = y = range(-5, 5, length = 40)                             #Definimos x & y dos variables que van desde -5 hasta 5.
	zs = zeros(0,40)                                              #Definimos zs una variable que vaya hasta la misma distancia que x & y.
	n = 200                                                      #Definimos una variable n que entre menor sea la gráfica se moverá más lenta.

@gif for i in range(0, stop = 2π, length = n)                     #Definimos el rango de la gráfica, donde n definirá su "rápidez".
	s(x,y) = cos(x + 10sin(i)) + sin(y)                           #Definimos la función s para que se mueva en "y".

p = plot(x, y, s, st =:wireframe)                                 #Gráficamos la función con las variables antes definidas.
	
	xlabel!("x")                                                  #Le damos el nombre de "x" al eje x.
    ylabel!("y")                                                  #Le damos el nombre de "y" al eje y.
	
fixed_x = zeros(40)
z = map(s, fixed_x, y)                                            #Definimos "z" como el mapeado de la función, x & y.



#Podemos ver que la gráfica se desplaza hacía el eje "y"
end
end
