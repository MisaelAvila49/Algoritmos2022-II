...
function aleatorioUniforme(a,b)   # Definimos la función aleatorioUniforme.
...
histogram([aleatorioUniforme(a,b)], legend = false,title = "Aleatorio Uniforme") #Hacemos un histograma de la función.
...
function unPaso()                                    # Definimos una función unPaso.
	
	return 2*rand(Bool) -1                           # Regresa dos veces rand de numeros Booleanos menos uno.
...
function variosPasos(n)                             # Definimos la función como variosPasos.
	
	return 2*rand(Bool,n).-1                        # Utilizamos la misma definición que en la anterior pero usando un punto para que sea un arreglo para cualquiera.
...
function caminataAleatoria(n)                        # Definimos la función caminataAleatoria.
	
	return variosPasos(n)                            # Regresamos a la función varios pasos.
	caminata = cumsum([0;pasos])                     # Definimos caminata como la suma de los elementos.
...
function graficaCaminata(caminataAleatoria)                                             # Definimos la función graficaCaminata.
	
	n = length(caminataAleatoria)                                                       # Definimos n como el tamaño de la función caminata.
	
	plot(1:n,caminataAleatoria,seriestype = :scatter, title= "Gráfica de caminata",     # Gráficamos la funcion de 1 hasta n.
	xlabel = "Número de pasos", ylabel = "Posición")                                             	
end
	
n2 = 150                                                                                # Definimos n2
caminataleatoria = caminataAleatoria(n2)                                                # Definimos caminata aleatoria para ese n2
graficaCaminata(caminataleatoria)                                                       # Graficamos la función.
...
function graficaCaminata!(caminataleato)                      # Definimos la función graficaCaminata!

	n = length(caminataleato)                                 # Definimos n como el tamaño de la caminata aleatoria.
    plot!(1:n, caminataleato, seriestype = :path)             # Graficamos desde 1 hasta n la caminata aleatoria.
...
n = 500                                                # Definimos una n.
	
for i in 1:5                                           # Para un valor i desde 1 hasta 5.
	
	caminataleato = caminataAleatoria(n)               # Definimos caminataleato = caminata Aleatoria para algun n.
...
function animaCaminata(caminatal)                      # Definimos la función animaCaminata para una caminata"l".
	
	n = length(caminatal)                              # Definimos n como el tamaño de la caminata"l".
anim = @animate for i in 1:n                           # Generamos una animación para i desde 1 hasta n.
	graficaCaminata(caminatal[1:i])                    # Usamos la función graficaCaminata para graficarla.
end
	
	return anim                                        # Regresamos a la función anim.
end
	
n3= 220                                                # Definimos una nueva n.
caminatal = caminataAleatoria(n3)                      # Definimos la caminata.
anima = animaCaminata(caminatal)                       # Definimos la función anima.
gif(anima, "caminatal.gif" , fps =20)                  # Generamos un gif con la funcion anima para la caminata.
end
...
function graficaCaminata2D(camino)                                                 # Definimos la función graficaCaminata2D.
	
x = [p[1] for p in camino]                                                         # Definimos x como p1 para p en el camino.
	y = [p[2] for p in camino]                                                     # definimos y como p2 para p en el camino.
	
plot(x, y, linestyle =:dash, aspect_radio = 1, title= "Gráfica caminatas en 2D")   # Graficamos x & y.
end
	
N3 = 579                                                                           # Definimos una nueva n.
camino = caminataAleatoria2D(N3)                                                   # Definimos camino como la caminata aleatoria en este n.
graficaCaminata2D(camino)                                                          # Usamos la función de graficaCaminata2D para camino.
...
function graficaCaminata2D!(Caminata,g)                                  # Definimos la función graficacaminata2d!
	x = [p[1] for p in Caminata]                                         # Definimos x.
	y = [p[2] for p in Caminata]                                         # Definimos y.
	
	plot!(g, x, y, linestyle=:dash, aspect_ratio=1)                      # Graficamos la función g y los puntos de x & y.
    display(g)                     
end
	n5= 7689                                                             # Definimos un nuevo n.
	
g = plot(title= "Cinco caminatas aleatorias", xlabel= "Coordenada x",
	ylabel = "Coordenada y", legend = false, aspect_ratio=1)             # Definimos g como la grafica de las cinco caminatas.

for i = 1:5                                                              # Para una i desde 1 hasta 5.
	Caminata = caminataAleatoria2D(n5)                                   # Definimos caminata como la caminata aleatoria en 2D de n.
	graficaCaminata2D!(Caminata,g)                                       # Usamos la función graficacaminata para la caminata y g.
end
	g
...
function animaCaminata2D(caminatal)                                                        # Definimos la función animaCaminta2D.
	
	animacion1 = plot(title="Caminata aleatoria en 2d", xlims=(-50,50), ylims= (-50,50))   # Definimos animacion1 como la grafica de la caminata aleatoria.
	
		for i in 1:length(caminatal)                                                       # Para i desde 1 hasta el tamaño de la caminata"l".
		graficaCaminata2D(caminatal[1:i])                                                  # Usamos grafiCacaminata2D para el camino desde 1 hasta i.
	end
	
	return animacion1                                                                      # Regresamos a la animacion1.
end

function graficaCaminata2D(caminatal, animacion1)                                          # Definimos la funcion grficaCaminata2D con una caminata y animación.
	...
	plot!(animacion1, x, y, color=:green, lw=1.5)                                          # Graficamos la animacion1.
	scatter!(animacion1, x[end], y[end], color=:red, ms=4)                                 # Dibuja puntos de la animacion1 y el punto final de x & de y.
end
	
	caminatal1 = [randn(2) for i in 1:100]                                                 # Definimos caminatal1 como numeros al azar para i desde 1 hasta 100.
	@gif for i in 1:length(caminatal1)                                                     # Creamos un gif para i desde 1 hasta el tamaño de la caminata.
		animaCaminata2D(caminatal1[1:i])                                                   # Usamos animaCaminata2D para la caminatal1.
end
...
begin
function caminataAleatoria3D(n)                          # Definimos la funcion caminata! aleatoria en 3D.
	
	caminata! = [[0.0, 0.0, 0.0]]                        # Definimos la caminata inicial.
	
	for i in 2:n                                         # Para una i desde 2 hasta n.
		
		paso = [randn(), randn(), randn()]               # Definimos el paso como un vector de 3 entradas con valores diversos
		nuevo_punto = caminata![i-1] + paso              # Definimos nuevo punto como la caminata! de i-1 sumando paso.
		push!(caminata!, nuevo_punto)                    # Arregla caminata! al final de la funcion nuevo punto.
...
function graficaCaminata3D(caminata!)                    # Definimos la función grafica para caminata! en 3D.
	
	x = [p[1] for p in caminata!]                        # Definimos x p1 para p en la caminata!.
	y = [p[2] for p in caminata!]                        # Definimos y p2 para p en la caminata!.
	z = [p[3] for p in caminata!]                        # Definimos z p3 para p en la caminata!.
	
scatter3d(x,y,z, markersize=2, markerstrokewidth =0,     # Graficamos en x,y & z.
	markeropacity= 0.5, legend= false )                  
...
n7 = 10780                                                # Definimos un nuevo n.
caminata! = caminataAleatoria3D(n)                        # Definimos caminata! como la caminata aleatoria en n.
graficaCaminata3D(caminata!)                              # Usamos la funcion graficaCaminata3d para caminata!.
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "b1e838f80cb805288a11d77068bdf5915c7c28df"
