...
function newton(f::Function,f′,x0,n)     # Definimos la funcion newton, con f la función, f' la derivada, x0 una aproximación inicial raíz de f y n el número de iteraciones.
	
	x = x0                               # Definimos x igual a x0.
	i = 0                                # Tomamos i como la primera interacción.
	
	while abs(f(x)) > i < n              # Mientras el valor aboluto de la función sea mayor que la iteracion y esta sea menor que el número de iteraciones.
...
function newtonRecursiva(f,df,x0,t,n)  # Definimos la función newtonRecursiva, con f la funcion, df la deivada, x0 un valor inicial, t una variable y n el numero de iteraciones.
	
	if n == 0 || abs(f(x0)) < t        # Si n es igual a 0 o el valor absoluto de la funcion en x0 es menor a t.
		
    return x0                          # Regresa a x0.
		
	else                               # En cualquier otro caso.
		
		x = x0 - f(x0) /df(x0)         # Definimos x como x0 - f(x0)/df(x0)
	return newtonRecursiva(f,df,t,n-1) # Regresa a la función newtonRecursiva pero esta vez con n-1 iteraciones.
...
throw(DimensionMismatch("Las dimensiones de los vectores deben ser iguales"))   # Imprime "Las dimensiones de los vectores deben ser iguales"
 end
	
	d = 0.0                                                                      # Definimos d la distancia 0.
	g = zeros(T, n)                                                              # Definimos g como el arreglo de ceros T,n.
	h = zeros(T,n,n)                                                             # Definimos h como el arreglo de ceros T,n,n.
	
for i in 1:n                                                                     # Para i de 1 hasta n.
	
	g[i] = f(x0)[i] - y[i]                                                       # La función g(i) es igual a f(x0)(i) -y(i).
	d += g[i]^2                                                                  # Sumamos d a g(i)^2.
end
	
	while true                                                                   # Mientras sea verdadero.
		
for i in 1:n, j in 1:n                                                           # Para i de 1 hasta n y j de 1 hasta n.
		
	h[i,j] = f′(x0,i)[j]                                                         # h(i,j) es igual a la derivada de (x0,i)(j)
end
		
  try                                                                            # Cuando z sea igual al inverso de h por g.
	  z = -inv(h)*g
  catch ex                                                                       # Sucede una excepción.
  end
		
if norm(z) < epsilon                                                             # Si la norma de z es menor a epsilon.
	
	 break                                                                       # Se interrumpe el while.
	end
		
		d0 = d                                                                   # Definimos d0 como la distancai d.
		
for i in 1:n                                                                     # Para i de 1 hasta n.
	
	x0[i] += z[i]                                                                # Suma x0(i) a z(i).
    g[i] = f(x0)[i] -y[i]                                                        # Definimos g(i) como f(x0)(i) -y(i).
	d += g[i]^2                                                                  # Suma d a g(i)^2.
end
		
if d >= d0                                                                       # Si d es mayor o igual a d0.
	
	return -1.0                                                                  # Regresa a -1.
end
end
	return sqrt(d)                                                               # Regresa a la raiz de la distancia.
end
...
while true                                    # Mientras sea verdadero.
	...
	fx = f(x)                                 # Definimos fx  como la función en x.
	derivadaNumérica = (f(x+h)-f(x))/h        # Definimos la derivada numérica.
...
if abs(derivadaNumérica) < 1e-9               # Si el valor absoluto de la derivada es menor a 1e-9.
	...
	return -1.0                               # Regresa a -1.
	 ...
	 x -= fx/derivadaNumérica                 # Resta de x y fx/ la derivada 
...
@bind x0 Slider(-10:0.1:10, default=0.1)           # Definimos un deslizador para x0 de -10 a 10 con pasos de 0.1.

# ╔═╡ 3efd83d2-e83b-4742-9e72-881c8e0ce80d
@bind h Slider(1000:1000:1000000, default=1000)    # Definimos un deslizador para h de 1000 a 1000000 con pasos de 1000.

# ╔═╡ 1c590251-c7d1-4b1e-9350-07cc2b189744
@bind n Slider(1:1000)                             # Definimos un deslizador para n de 1 a 1000 con pasos de 1

# ╔═╡ 005372a4-35cc-4d81-846e-0aa9b66a6ce5
f(x) = cos(x)                                      # Luego se usa esta celda para reasignar `f`.                   
...
xi = [newtonDN(f, x0, h,i) for i in 1:n]                  # Definimos xi como el valor de la función newtonDN.
	
fi = f.(xi)                                               # Definimos f para un xi.
	
scatter(xi, fi, marker=:circle, line=:true, label="",     # Graficamos los puntos. 
xlabel ="x", ylabel="f(x)", title="Gráfica Newton")       
	plot!(f)                                              # Graficamos la función f.
