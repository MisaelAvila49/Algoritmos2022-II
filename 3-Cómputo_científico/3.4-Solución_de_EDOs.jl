...
deslizador = @bind C Slider(-10:0.1:10, default = 1) #Creamos un slider con una variable C, con valores de -10 a 10 con un paso de 0.1.

# ╔═╡ 5c5e85dd-8685-46fc-92fe-416ca717e696
C               # Imprimimos el valor de C.

# ╔═╡ 1418e9a9-2253-4e03-a627-e64b13a3682a
x(t) = C*exp(t) # Definimos una función x(t) que sea igual a Ce^t con el parámetro de la familia interactivo C.

# ╔═╡ 63e3150c-18d2-4db6-8208-810a5eb59b8c
begin
	
    plot(-1:0.1:9, x, ylims = (-1000,1000), label = L"$Ce^t$")   # Graficamos a la función x de -1 a 9 con pasos de 0.1.
	vline!([0,0], color = "grey", style = :dash, label = false)  # Agregamos una línea punteada vertical de color gris.
	hline!([0,0], color = "grey", style = :dash, label = false)  # Agregamos una línea punteada horizontal de color gris.
...
deslizador # Mostramos de nuevo el deslizador interacttivo.
...
begin
    ...
    plot(-1:0.05:1, x, ylims = (-10,10), label = L"$Ce^t$")       # Graficamos la función x de -1 a 9 con pasos de 0.05.
	vline!([0,0], color = "grey", style = :dash, label = false)   # Agregamos una línea punteada vertical de color gris.
	hline!([0,0], color = "grey", style = :dash, label = false)   # Agregamos una línea punteada horizontal de color gris.
...
function tamañoDePaso(t0,tf,N)           # Definimos la función tamaño de paso.
	
  h = (tf-t0)/N                          # Definimos h como (tf-t0)/N
	
	return h                             # Regresa a h
end
...
function arregloUniforme(f,t0,tf,N,h)          # Definimos la función arregloUniforme, con f la funcion, t0 el tiempo inicia, tf el tiempo final y h el tamaño de paso.
	
if N <= 0                                      # Si N es menor o igual a cero.
	
	error("Debe ser un entero positivo")       # Imprime el mensaje de error "Debe ser un entero positivo".
end	
	
	h = (tf-t0)/N                              # Definimos h como el tamaño de paso.
    x = collect(x0:h:xf)                       # Definimos x como una colección de x0 hasta xf con un tamaño de paso h.
	y = zeros(n+1)                             # Definimos y como un arreglo de ceros de n+1.
	y[1] = y0                                  # Definimos y1 = y0

for i in 1:n                                   # Para i desde 1 hasta n.
	
	y[i+1] = y[i] +h * f(x[i], y[i])           # Definimos y[i+1] = y[i] + h * f(x[i],y[i]).
end
	return(x,y)                                # Regresa a (x,y).
end
...
function paso_euler(ti,xti,g,h)      # Definimos la función paso_euler con ti el tiempo, xti una aproximación de x, g una función de x, h el tamaño del arreglo uniforme.
	
	xti1 = xti + h *g(ti,xti)        # Definimos xti1 como xti + h *g(ti,xti).
...
begin
	function euler(g,x0,t)                              # Definimos la funcion euler usando g función de x y t, x0  la condición inicial, t el arreglo uniforme tiempos.
		
    n = length(t)                                       # Definimos n el tamaño de t.
	h = t[2] -t[1]                                      # Definimos h como la diferencia entre t2 y t1.
	xt = zeros(n)                                       # Definimos xt un arreglo de ceros de n.
	xt[1] = t0                                          # Definimos xt[1] = t0.

	for i in 1:n-1                                      # Para i desde 1 hasta n-1.
		
		xt[i+1] =xt[i] +h * g(t[i], xt[i])              # Definimos xt[i+1] como xt[i] + h * g(t[i], xt[i])
	end
		
	 return xt                                          # Regresamos xt
end
	
function m(l, x)                                        # Ahora definimos una funcion que solo depende de una variable.
	
    return 2*l                                          # Regresamos a 2*l
end
	
t0 = 1.0                                                # Colocamos el tiempo inicial.
l = range(0.0, 1.0, step=0.1)                           # Definimos la l.
